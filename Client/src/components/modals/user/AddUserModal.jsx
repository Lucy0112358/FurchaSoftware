...
import { clearFilteredLockerGroups, getAllLockersData, getFilteredLockerGroups } from "../../../redux/slice/lockerSlice";
...
const AddUserModal = ({ isOpen, onClose, mode = "add", initialData = {} }) => {
  ...
  const getLockersData = useSelector(getAllLockersData);
  ...
  useEffect(() => {
    if (mode === "edit" && initialData) {
      setSentGeneralInfo({
        ...
        activeFrom: initialData.activeFrom ? initialData.activeFrom.split("T")[0] : '',
        activeTo: initialData.activeTo ? initialData.activeTo.split("T")[0] : '',
      });
        ...
        <Tabs>
          ...
          <TabPanel>
            ...
            <div className="flex justify-end space-x-4 mt-4">
              <button className="bg-gray-600 text-white rounded px-4 py-2" onClick={handleClose}>
                Cancel
              </button>
                      ...
                      <React.Fragment key={groupIndex}>
                        ...
                        <div className="flex flex-wrap mb-4">
                          {lockerGroup.groupLockers.map((item, itemIndex) => (
                            <div
                              key={itemIndex}
                              className={`mr-2 mb-2 ${selectedLockerId.includes(item.id) ? 'selected__branch__id' : 'locker__border'}`}
                              onMouseOver={(event) => {
